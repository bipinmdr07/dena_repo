class LessonsController < ApplicationController
  ...
  def show
    ...
    @paginated_submissions = Submission.where(course_name: current_controller, lesson_id: @current_lesson_id)
                                       .order("created_at DESC")
                                       .paginate(page: params[:page], per_page: 5)

    @all_submissions = Submission.where(course_name: current_controller, lesson_id: @current_lesson_id)
                                 .order("created_at DESC")

    @submissions = @all_submissions.approved
